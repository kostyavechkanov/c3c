...
#define EOUTPUT(string, ...)                                                   \
  fprintf(stderr, string "\n", ##__VA_ARGS__) // NOLINT
#define PRINTF(string, ...)                                                    \
  fprintf(stdout, string "\n", ##__VA_ARGS__) // NOLINT
#define FAIL_WITH_ERR(string, ...)                                             \
  do {                                                                         \
    fprintf(stderr, "Error: " string "\n\n", ##__VA_ARGS__);                   \
    usage();                                                                   \
    exit_compiler(EXIT_FAILURE);                                               \
  } while (0) /* NOLINT */
#define PROJECT_FAIL_WITH_ERR(string, ...)                                     \
  do {                                                                         \
    fprintf(stderr, "Error: " string "\n\n", ##__VA_ARGS__);                   \
    project_usage();                                                           \
    exit_compiler(EXIT_FAILURE);                                               \
...
static void usage(void) {
  ...
  PRINTF("Commands:");
  PRINTF("");
  PRINTF("  compile <file1> [<file2> ...]                       Compile files "
         "without a project into an executable.");
  PRINTF("  init <project name>                                 Initialize a "
         "new project structure.");
  PRINTF("  init-lib <library name>                             Initialize a "
         "new library structure.");
  PRINTF("  build [<target>]                                    Build the "
         "target in the current project.");
  PRINTF("  benchmark                                           Run the "
         "benchmarks in the current project.");
  PRINTF("  test                                                Run the unit "
         "tests in the current project.");
  PRINTF("  clean                                               Clean all "
         "build files.");
  PRINTF("  run [<target>]                                      Run (and build "
         "if needed) the target in the current project.");
  PRINTF("  dist [<target>]                                     Clean and "
         "build a target for distribution.");
  PRINTF("  directives [<target>]                               Generate "
         "documentation for the target.");
  PRINTF("  bench [<target>]                                    Benchmark a "
         "target.");
  PRINTF("  clean-run [<target>]                                Clean, then "
         "run the target.");
  PRINTF("  compile-run <file1> [<file2> ...] [-- [<arg1> ...]] Compile files "
         "then immediately run the result.");
  PRINTF("  compile-only <file1> [<file2> ...]                  Compile files "
         "but do not perform linking.");
  PRINTF("  compile-benchmark <file1> [<file2> ...]             Compile files "
         "into an executable and run benchmarks.");
  PRINTF("  compile-test <file1> [<file2> ...]                  Compile files "
         "into an executable and run unit tests.");
  PRINTF("  static-lib <file1> [<file2> ...]                    Compile files "
         "without a project into a static library.");
  PRINTF("  dynamic-lib <file1> [<file2> ...]                   Compile files "
         "without a project into a dynamic library.");
  PRINTF("  headers <file1> [<file2> ...]                       Analyse files "
         "and generate C headers for public methods.");
  PRINTF("  vendor-fetch <library> ...                          Fetches one or "
         "more libraries from the vendor collection.");
  PRINTF("  project <subcommand> ...                            Manipulate or "
         "view project files.");
  PRINTF("");
  PRINTF("Options:");
  PRINTF("  --tb                       - Use Tilde Backend for compilation.");
  PRINTF("  --stdlib <dir>             - Use this directory as the C3 standard "
         "library path.");
  PRINTF("  --no-entry                 - Do not generate (or require) a main "
         "function.");
  PRINTF("  --libdir <dir>             - Add this directory to the C3 library "
         "search paths.");
  PRINTF("  --lib <name>               - Add this library to the compilation.");
  PRINTF("  --path <dir>               - Use this as the base directory for "
         "the current command.");
  PRINTF("  --template <template>      - Select template for 'init': \"exe\", "
         "\"static-lib\", \"dynamic-lib\" or a path.");
  ...
  PRINTF("  --run-once                 - After running the output file, delete "
         "it immediately.");
  ...
  PRINTF("  -O0                        - Safe, no optimizations, emit debug "
         "info.");
  PRINTF("  -O1                        - Safe, high optimization, emit debug "
         "info.");
  PRINTF("  -O2                        - Unsafe, high optimization, emit debug "
         "info.");
  PRINTF("  -O3                        - Unsafe, high optimization, single "
         "module, emit debug info.");
  PRINTF("  -O4                        - Unsafe, highest optimization, relaxed "
         "maths, single module, emit debug info, no panic messages.");
  PRINTF("  -O5                        - Unsafe, highest optimization, fast "
         "maths, single module, emit debug info, no panic messages, no "
         "backtrace.");
  PRINTF("  -Os                        - Unsafe, high optimization, small "
         "code, single module, no debug info, no panic messages.");
  PRINTF("  -Oz                        - Unsafe, high optimization, tiny code, "
         "single module, no debug info, no panic messages, no backtrace.");
  PRINTF("  -D <name>                  - Add feature flag <name>.");
  PRINTF("  -U <name>                  - Remove feature flag <name>.");
  PRINTF("  --trust=<option>           - Trust level: none (default), include "
         "($include allowed), full ($exec / exec allowed).");
  ...
  PRINTF("  --llvm-out <dir>           - Override llvm output directory for "
         "'--emit-llvm'.");
  PRINTF(
      "  --emit-llvm                - Emit LLVM IR as a .ll file per module.");
  PRINTF("  --asm-out <dir>            - Override asm output directory for "
         "'--emit-asm'.");
  PRINTF("  --emit-asm                 - Emit asm as a .s file per module.");
  PRINTF(
      "  --obj                      - Emit object files. (Enabled by default)");
  PRINTF("  --no-obj                   - Do not output object files, this is "
         "only valid for `compile-only`.");
  PRINTF("  --no-headers               - Do not generate C headers when "
         "building a library.");
  PRINTF("  --target <target>          - Compile for a particular architecture "
         "+ OS target.");
  PRINTF("  --threads <number>         - Set the number of threads to use for "
         "compilation.");
  PRINTF("  --safe=<yes|no>            - Turn safety (contracts, runtime "
         "bounds checking, null pointer checks etc) on or off.");
  PRINTF("  --panic-msg=<yes|no>       - Turn panic message output on or off.");
  PRINTF("  --optlevel=<option>        - Code optimization level: none, less, "
         "more, max.");
  PRINTF("  --optsize=<option>         - Code size optimization: none, small, "
         "tiny.");
  PRINTF("  --single-module=<yes|no>   - Compile all modules together, enables "
         "more inlining.");
  PRINTF(
  ...
  PRINTF("  -L <library dir>           - Append the directory to the linker "
         "search paths.");
  PRINTF("  -z <argument>              - Send the <argument> as a parameter to "
         "the linker.");
  PRINTF("  --cc <path>                - Set C compiler (for C files in "
         "projects and use as system linker).");
  PRINTF("  --linker=<option> [<path>] - Linker: builtin, cc, custom (default "
         "is 'cc'), 'custom' requires a path.");
  PRINTF("");
  PRINTF("  --use-stdlib=<yes|no>      - Include the standard library "
         "(default: yes).");
  PRINTF("  --link-libc=<yes|no>       - Link libc other default libraries "
         "(default: yes).");
  PRINTF("  --emit-stdlib=<yes|no>     - Output files for the standard "
         "library. (default: yes)");
  PRINTF("  --panicfn <name>           - Override the panic function name.");
  PRINTF(
      "  --testfn <name>            - Override the test runner function name.");
  PRINTF("  --benchfn <name>           - Override the benchmark runner "
         "function name.");
  PRINTF("");
  PRINTF("  --reloc=<option>           - Relocation model: none, pic, PIC, "
         "pie, PIE.");
  PRINTF("  --x86cpu=<option>          - Set general level of x64 cpu: "
         "baseline, ssse3, sse4, avx1, avx2-v1, avx2-v2 (Skylake/Zen1+), "
         "avx512 (Icelake/Zen4+), native.");
  PRINTF("  --x86vec=<option>          - Set max type of vector use: none, "
         "mmx, sse, avx, avx512, default.");
  PRINTF("  --riscvfloat=<option>      - Set type of RISC-V float support: "
         "none, float, double");
  PRINTF("  --memory-env=<option>      - Set the memory environment: normal, "
         "small, tiny, none.");
  PRINTF("  --strip-unused=<yes|no>    - Strip unused code and globals from "
         "the output. (default: yes)");
  PRINTF("  --fp-math=<option>         - FP math behaviour: strict, relaxed, "
         "fast.");
  ...
  PRINTF("  --list-project-properties  - List all available keys used in "
         "project.json files.");
  PRINTF("  --list-manifest-properties - List all available keys used in "
         "manifest.json files.");
  PRINTF("  --list-targets             - List all architectures the compiler "
         "supports.");
  PRINTF("  --list-type-properties     - List all type properties.");
  PRINTF("");
  PRINTF("  --print-output             - Print the object files created to "
         "stdout.");
  PRINTF("  --print-input              - Print inputted C3 files to stdout.");
  PRINTF("");
  PRINTF("  --winsdk <dir>             - Set the directory for Windows system "
         "library files for cross compilation.");
  PRINTF("  --wincrt=<option>          - Windows CRT linking: none, static, "
         "dynamic (default).");
  PRINTF("  --windef <file>            - Use Windows 'def' file for function "
         "exports instead of 'dllexport'.");
  PRINTF("");
  PRINTF("  --macossdk <dir>           - Set the directory for the MacOS SDK "
         "for cross compilation.");
  PRINTF("  --macos-min-version <ver>  - Set the minimum MacOS version to "
         "compile for.");
  PRINTF("  --macos-sdk-version <ver>  - Set the MacOS SDK compiled for.");
  PRINTF("");
  PRINTF("  --linux-crt <dir>          - Set the directory to use for finding "
         "crt1.o and related files.");
  PRINTF("  --linux-crtbegin <dir>     - Set the directory to use for finding "
         "crtbegin.o and related files.");
  PRINTF("");
  PRINTF("  --vector-conv=<option>     - Set vector conversion behaviour: "
         "default, old.");
}
...
static const char *check_dir(const char *path) {
  ...
  if (!dir_change(path))
    error_exit("The path \"%s\" does not point to a valid directory.", path);
  if (!dir_change(original_path))
    FAIL_WITH_ERR("Failed to change path to %s.", original_path);
  return path;
}
...
static void print_version(void) {
  PRINTF("C3 Compiler Version:       %s%s", COMPILER_VERSION,
...
static void parse_option(BuildOptions *options) {
  ...
  case 'z':
    if (match_shortopt("z")) {
      if (at_end())
        error_exit("error: -z needs a value.");
  ...
  case 'o':
    if (match_shortopt("o")) {
      if (at_end())
        error_exit("error: -o needs a name.");
    ...
    if (match_shortopt("D")) {
      ...
      if (!str_is_valid_constant(arg))
        error_exit("Invalid feature name '%s', expected an all-uppercase "
                   "constant name.",
                   arg);
      update_feature_flags(&options->feature_names,
                           &options->removed_feature_names, arg, true);
      return;
    }
    break;
  case 'U':
    if (match_shortopt("U")) {
      ...
      if (!str_is_valid_constant(arg))
        error_exit("Invalid feature name '%s', expected an all-uppercase "
                   ...
                   arg);
      update_feature_flags(&options->feature_names,
                           &options->removed_feature_names, arg, false);
      return;
    }
    break;
  case 'O':
  ...
  case 'L':
    if (match_shortopt("L")) {
      if (at_end() || next_is_opt())
        error_exit("error: -L needs a directory.");
      options->linker_lib_dir[options->linker_lib_dir_count++] =
          check_dir(next_arg());
      return;
    }
    break;
  case 'l':
    if (match_shortopt("l")) {
      if (at_end() || next_is_opt())
        error_exit("error: -l needs a library name.");
  ...
  case '-':
    ...
    if ((argopt = match_argopt("optsize"))) {
      options->optsize =
          (SizeOptimizationLevel)parse_multi_option(argopt, 3, optsizes);
      return;
    }
    if ((argopt = match_argopt("optlevel"))) {
      options->optlevel =
          (OptimizationLevel)parse_multi_option(argopt, 4, optlevels);
      return;
    }
    if ((argopt = match_argopt("safe"))) {
      options->safety_level =
          (SafetyLevel)parse_multi_option(argopt, 2, on_off);
      return;
    }
    if ((argopt = match_argopt("show-backtrace"))) {
      options->show_backtrace =
          (ShowBacktrace)parse_multi_option(argopt, 2, on_off);
      return;
    }
    ...
    if ((argopt = match_argopt("single-module"))) {
      options->single_module =
          (SingleModule)parse_multi_option(argopt, 2, on_off);
      return;
    }
    if ((argopt = match_argopt("linker"))) {
      ...
      if (options->linker_type == LINKER_TYPE_CUSTOM) {
        if (at_end() || next_is_opt())
          error_exit("error: --linker=custom expects a valid linker name.");
    ...
    if ((argopt = match_argopt("strip-unused"))) {
      options->strip_unused =
          (StripUnused)parse_multi_option(argopt, 2, on_off);
      return;
    }
    ...
    if ((argopt = match_argopt("x86vec"))) {
      options->x86_vector_capability = (X86VectorCapability)parse_multi_option(
          argopt, 6, x86_vector_capability);
      return;
    }
    if ((argopt = match_argopt("win64-simd"))) {
      options->win_64_simd =
          (Win64Simd)parse_multi_option(argopt, 2, win64_simd_type);
      return;
    }
    if ((argopt = match_argopt("x86cpu"))) {
      options->x86_cpu_set =
          (X86CpuSet)parse_multi_option(argopt, 8, x86_cpu_set);
      return;
    }
    if ((argopt = match_argopt("riscvfloat"))) {
      options->riscv_float_capability =
          (RiscvFloatCapability)parse_multi_option(argopt, 3, riscv_capability);
      return;
    }
    if ((argopt = match_argopt("memory-env"))) {
      options->memory_environment =
          (MemoryEnvironment)parse_multi_option(argopt, 4, memory_environment);
      return;
    }
    if ((argopt = match_argopt("reloc"))) {
      options->reloc_model =
          (RelocModel)parse_multi_option(argopt, 5, reloc_models);
      return;
    }
    ...
    if (match_longopt("threads")) {
      ...
      if (threads > MAX_THREADS)
        PRINTF("Cannot exceed %d threads.", MAX_THREADS);
    ...
    if (match_longopt("cc")) {
      if (at_end() || next_is_opt())
        error_exit("error: --cc needs a compiler name.");
      options->cc = next_arg();
      return;
    }
    if (match_longopt("stdlib")) {
      if (at_end() || next_is_opt())
        error_exit("error: --stdlib needs a directory.");
    ...
    if (match_longopt("panicfn")) {
      if (at_end() || next_is_opt())
        error_exit("error: --panicfn needs a function name.");
      options->panicfn = next_arg();
      return;
    }
    if (match_longopt("testfn")) {
      if (at_end() || next_is_opt())
        error_exit("error: --testfn needs a function name.");
      options->testfn = next_arg();
      return;
    }
    if (match_longopt("benchfn")) {
      if (at_end() || next_is_opt())
        error_exit("error: --benchfn needs a function name.");
      options->benchfn = next_arg();
      return;
    }
    if (match_longopt("macossdk")) {
      if (at_end() || next_is_opt())
        error_exit("error: --macossdk needs a directory.");
      options->macos.sysroot = check_dir(next_arg());
      return;
    }
    if (match_longopt("winsdk")) {
      if (at_end() || next_is_opt())
        error_exit("error: --winsdk needs a directory.");
      options->win.sdk = check_dir(next_arg());
      return;
    }
    if ((argopt = match_argopt("trust"))) {
      options->trust_level =
          (TrustLevel)parse_multi_option(argopt, 3, trust_level);
      return;
    }
    if (match_longopt("windef")) {
      if (at_end() || next_is_opt())
        error_exit("error: --windef needs a file.");
      options->win.def = next_arg();
      return;
    }
    if ((argopt = match_argopt("vector-conv"))) {
      options->vector_conv =
          (VectorConv)parse_multi_option(argopt, 2, vector_conv);
      return;
    }
    if ((argopt = match_argopt("wincrt"))) {
      options->win.crt_linking =
          (WinCrtLinking)parse_multi_option(argopt, 3, wincrt_linking);
      return;
    }
    if (match_longopt("macos-sdk-version")) {
      if (at_end() || next_is_opt())
        error_exit("error: --macos-sdk-version needs a version.");
      options->macos.sdk_version = next_arg();
      return;
    }
    if (match_longopt("macos-min-version")) {
      if (at_end() || next_is_opt())
        error_exit("error: --macos-min-version needs a version.");
      options->macos.min_version = next_arg();
      return;
    }
    if (match_longopt("output-dir")) {
      if (at_end() || next_is_opt())
        error_exit("error: --output-dir needs a directory.");
      options->output_dir = next_arg();
      return;
    }
    if (match_longopt("build-dir")) {
      if (at_end() || next_is_opt())
        error_exit("error: --build-dir needs a directory.");
      options->build_dir = next_arg();
      return;
    }
    if (match_longopt("obj-out")) {
      if (at_end() || next_is_opt())
        error_exit("error: --obj-out needs a directory.");
      options->obj_out = next_arg();
      return;
    }
    if (match_longopt("script-dir")) {
      if (at_end() || next_is_opt())
        error_exit("error: --script-dir needs a directory.");
      options->script_dir = next_arg();
      return;
    }
    if (match_longopt("llvm-out")) {
      if (at_end() || next_is_opt())
        error_exit("error: --llvm-out needs a directory.");
      options->llvm_out = next_arg();
      return;
    }
    if (match_longopt("asm-out")) {
      if (at_end() || next_is_opt())
        error_exit("error: --asm-out needs a directory.");
      ...
      if (!str_is_valid_lowercase_name(name)) {
        if (str_has_suffix(name, ".c3l")) {
          error_exit("When specifying libraries, the .c3l suffix should"
                     " not be included, so rather than '--lib %s', try using "
                     "'--lib %s' instead.",
        ...
        if (str_has_suffix(name, ".lib") || str_has_suffix(name, ".a") ||
          ...
          error_exit("You tried to add '%s' as a C3 library, but from the name "
                     "it appears to be a"
                     " static/dynamic library. To link with such a library, "
                     "use '-l <name>' instead.",
                     name);
        }
        ...
        if (strchr(name, '/') != NULL ||
          ...
          error_exit(
              "There is a problem including the library '%s': a library name "
              "should never contain the path. Use '--libdir' to add the "
              "directory to the library search paths, then use the plain name "
              "for '--lib', "
              "e.g '--libdir my_project/libs --lib some_lib'.",
              name_copy);
        }
        error_exit(
            "Invalid library name '%s', it should be something like 'foo_lib'.",
            name_copy);
      }
    ...
    if (match_longopt("libdir")) {
      if (at_end() || next_is_opt())
        error_exit("error: --libdir needs a directory.");
      if (options->lib_dir_count == MAX_LIB_DIRS)
        error_exit("Max %d library directories may be specified.",
                   MAX_LIB_DIRS);
    ...
    if (match_longopt("template")) {
      if (at_end() || next_is_opt())
        error_exit("error: --template needs an argument.");
      options->template = next_arg();
      return;
    }
    if (match_longopt("path")) {
      if (at_end() || next_is_opt())
        error_exit("error: --path needs a directory.");
      options->path = check_dir(next_arg());
      return;
    }
    if (match_longopt("linux-crt")) {
      if (at_end() || next_is_opt())
        error_exit("error: --linux-crt needs a directory.");
      options->linuxpaths.crt = check_dir(next_arg());
      return;
    }
    if (match_longopt("linux-crtbegin")) {
      if (at_end() || next_is_opt())
        error_exit("error: --linux-crtbegin needs a directory.");
